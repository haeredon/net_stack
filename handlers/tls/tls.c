...
void tls_write_server_hello(struct tls_priv_t* priv, struct tls_client_hello_t* client_hello) {
    ...
    uint8_t* out_data = (uint8_t*) out + sizeof(struct tls_server_hello_t);
    ...
    memcpy(out_data, client_legacy_session, server_hello_offset);
    ...
    struct tls_ciphersuites_t* server_cipher_suite = (struct tls_ciphersuites_t*) ((uint8_t*) out_data + server_hello_offset);
    server_cipher_suite->num_bytes = htons(2);
    server_cipher_suite->suites[0] = selected_cipher_suite;

    // set compression methods
    *((uint8_t*) out_data + server_hello_offset) = 0;
    server_hello_offset += 1;

    // start extensions
    void* extensions = start_extensions((uint8_t*) out_data + server_hello_offset);
    uint16_t extensions_offset = 0;
    ...
    extensions_offset += tls_write_server_hello_supported_version(&version, (uint8_t*) extensions);
    ...
    extensions_offset += tls_write_server_hello_key_share(selected_key_share, tls_generate_key_share, (uint8_t*) extensions + extensions_offset);

    uint16_t extensions_length = end_extensions(extensions, extensions_offset);

    buffer->offset += server_hello_offset + extensions_length;    
}
...
void tls_write_certificate_finished(struct tls_out_buffer* buffer) {
...
void tls_handle_handshake(struct tls_priv_t* priv, struct tls_handshake_msg_t* handshake) {
    ...
    switch (priv->control_block.state) {
        case TLS_SERVER_START:
            ...
            struct tls_client_hello_t* client_hello = (struct tls_client_hello_t*) ((uint8_t*) handshake + sizeof(struct tls_handshake_msg_t));

            tls_write_server_hello(priv, client_hello);            
...
uint8_t tls_handle_record(struct tls_record_t* record, struct tls_priv_t* priv) {
    switch(record->content_type) {
        case TLS_RECORD_CONTENT_TYPE_HANDSHAKE:
            tls_handle_handshake(priv, record);
            return 0;
    }
...
uint16_t tls_read(struct in_packet_stack_t* packet_stack, struct interface_t* interface, struct handler_t* handler) {
    ...
    while(bytes_left >= sizeof(struct tls_record_t)) {
        ...
        uint8_t fail = tls_handle_record(record, &priv);
