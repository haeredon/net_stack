...
int pcapng_read_block(struct pcapng_reader_t* reader, void* buffer, uint32_t max_size) {
    ...
    uint8_t block_buffer[HEADER_TYPE_LENGTH]; // to get block length
    ...
    memcpy(buffer, block_buffer, HEADER_TYPE_LENGTH);

    struct pcapng_block_type_length_t* type_length = (struct pcapng_block_type_length_t*) block_buffer;
    uint32_t num_to_read = type_length->block_length - HEADER_TYPE_LENGTH;
