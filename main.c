...
// 	*((uint64_t *)tmp) = 0x000000000002 + ((uint64_t)0 << 40);

// 	rte_ether_addr_copy(&l2fwd_ports_eth_addr[1], &eth->src_addr);
// }
...
0x04, 0xd4, 0xc4, 0x52, 0x7d, 0x67, 0x6c, 0xb3, /* ...R}gl. */
0x11, 0x57, 0x84, 0xbb, 0x08, 0x06, 0x00, 0x01, /* ...\.... */
...
static void
l2fwd_simple_forward(struct rte_mempool * pool) {
int i = 0;
	...
	while(i++ != 35){
		...
		memcpy(new_packet_block, &pkt47, 60);


		struct rte_eth_dev_tx_buffer *buffer = tx_buffer[0];

		int sent = rte_eth_tx_buffer(0, 0, buffer, packet);

		if(sent) {
			printf("Sent %d packet\n", sent);		
			printf("Dropped: %d, %d\n", port_statistics[0].dropped, port_statistics[1].dropped);
		}	
	}
...
// 	reader->file = fopen("kage.pcap", "rb");
...
// 				struct rte_pcapng* ret = rte_pcapng_fdopen(fd, NULL, NULL, NULL, NULL);

// 				if(ret == NULL) {
// 					printf("\nFAIL1\n");
// 				}

// 				int bla = rte_pcapng_add_interface(ret, 0, NULL, NULL, NULL);

// 				struct rte_mempool* pool = rte_pktmbuf_pool_create("pcap_pool", 8192U,
// 				MEMPOOL_CACHE_SIZE, 0, RTE_MBUF_DEFAULT_BUF_SIZE,
// 				rte_socket_id());
...
// 				struct rte_mbuf* buffers[1];
// 				buffers[0] = rte_pcapng_copy(portid, qconf->rx_port_list[i], m, pool, 8192U, RTE_PCAPNG_DIRECTION_IN, "");

// 				if(buffers[0] == NULL) {
// 					printf("\nFAIL3\n");
// 				}

// 				int num = rte_pcapng_write_packets(ret, buffers, 1);

// 				uint64_t kagea =  rte_errno;

// 				if(num < 0) {
// 					printf("\nFAIL4 %u \n", errno );
// 				}

// 				rte_pcapng_close(ret);
...
main(int argc, char **argv)
{
	...
	// 	if (qconf != &lcore_setups[rx_lcore_id].lcore_queue_conf) {
	// 		/* Assigned a new logical core in the loop above. */
	// 		qconf = &lcore_setups[rx_lcore_id].lcore_queue_conf;
	// 		struct handlers_t handlers = {
	// 			.read_handle = read
	// 		};
	// 		lcore_setups->handlers = handlers;
	// 		nb_lcores++;
	// 	}
